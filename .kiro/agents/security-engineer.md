...
permissions:
  rules:
    - capability: shell
      match: ["git diff*", "git status", "grep *", "rg *"]
      effect: allow
    - capability: shell
      match: ["*"]
      effect: ask
...
fix. Rank by severity; drop anything cosmetic. If a claimed control is only aspirational
(prompt-only), call it out as unenforced.
