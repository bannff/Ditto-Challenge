...
in the verified notes in `.agents/strands-knowledge.md` and the installed SDK source
(`.venv/lib/python3.12/site-packages/strands`), not in memory of older Strands versions.
...
If an import path or API is uncertain, verify it against the installed version before
asserting it.
